...
def train_and_predict() -> None:
    ...
    y = np.append(y, np.tile(Symbol.BETA.one_hot_encode(), (dataset_alpha.shape[1], 1)).T, axis=1)
    y = np.append(y, np.tile(Symbol.GAMMA.one_hot_encode(), (dataset_alpha.shape[1], 1)).T, axis=1)
    y = np.append(y, np.tile(Symbol.DELTA.one_hot_encode(), (dataset_alpha.shape[1], 1)).T, axis=1)
    y = np.append(y, np.tile(Symbol.EPSILON.one_hot_encode(), (dataset_alpha.shape[1], 1)).T, axis=1)

    nn = NeuralNetwork()
    nn.train(50000, combined_dataset, y)
